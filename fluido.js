/* =====================================================================
   A CORUJA IMPRESSA — revelação por simulação de fluido
   Apresentação de Prolog · Grupo 4

   O QUE ACONTECE AQUI
   O ponteiro faz duas coisas ao mesmo tempo:

     1. injeta tinta e velocidade numa simulação de fluido rodando na GPU
        (Stable Fluids, Jos Stam). Onde a tinta fica densa, a coruja é
        IMPRESSA no papel: retícula de meio-tom, chapa preta e chapa
        vermelha fora de registro. Como a tinta é carregada pela
        velocidade e vai dissipando, a impressão escorre e some sozinha.

     2. a posição horizontal controla o TEMPO do vídeo, e o vídeo é a
        coruja girando a cabeça da esquerda para a direita. Ou seja: ela
        acompanha o cursor.

   É a metáfora do trabalho: a inferência IMPRIME o que provou, e o que
   ela não visitou continua em branco na folha.

   MÉTODO
   Um fragment shader só enxerga o próprio pixel e não pode ler e
   escrever a mesma textura. Por isso cada etapa da física é um passe
   separado, lendo uma textura e escrevendo outra (ping-pong de FBOs).

   A ordem por quadro:
     splat -> advecção da velocidade -> advecção da tinta ->
     divergência -> pressão (Jacobi 20x) -> gradiente -> revelação

   Não há passe de vorticidade. A referência de produção usa
   curlStrength = 0, ou seja, o passe roda multiplicado por zero: o
   fluido liso é decisão de projeto, não defeito. Cortar os dois passes
   dá o mesmo resultado com menos código.

   AVISO QUE CUSTOU CARO
   Fluido WebGL quebra em SILÊNCIO: gl.getError() limpo e shaders
   compilando não provam nada. Cada correção abaixo marcada com
   [ARMADILHA] veio de renderizar e comparar quadros.
   ===================================================================== */

window.iniciarCoruja = function (opcoes) {
  ...
  var REVELACAO = CABECA + [
    ...
    "uniform float uDpr;",
    "",
    "const vec3 PAPEL = vec3(0.914, 0.890, 0.827);",
    "// O que cada tinta DEIXA passar. Nao sao tintas puras de teoria:",
    "// sao sujas como as de oficina, o que impede a cor de parecer",
    "// gradiente digital.",
    "const vec3 TINTA_C = vec3(0.16, 0.66, 0.80);",
    "const vec3 TINTA_M = vec3(0.88, 0.20, 0.48);",
    "const vec3 TINTA_Y = vec3(0.98, 0.87, 0.20);",
    "const vec3 TINTA_K = vec3(0.15, 0.14, 0.12);",
    "",
    ...
    "}",
    "",
    "// A reticula de uma chapa. A grade e em coordenada de TELA: se",
    "// acompanhasse a imagem, o ponto mudaria de tamanho junto com ela e",
    "// o olho perceberia que nao e impressao.",
    "float chapa(float valor, float angulo, float passo, vec2 registro) {",
    "  float c = cos(angulo), sn = sin(angulo);",
    "  vec2 g = mat2(c, -sn, sn, c) * ((vUv * uResolucao) + registro) / passo;",
    "  vec2 celula = fract(g) - 0.5;",
    "  float raio = sqrt(clamp(valor, 0.0, 1.0)) * 0.78;",
    "  return 1.0 - smoothstep(raio - 0.10, raio + 0.10, length(celula));",
    "}",
    "",
    "void main() {",
    "  vec2 uv = coverUv(vUv, uProporcaoImagem, uProporcaoTela);",
    "",
    "  // As tres linhas de sempre: a tinta do fluido e a mascara.",
    ...
    "",
    "  // A foto e de baixa luz: sem levantar preto e saturar, a chapa sai",
    "  // lavada e a coruja nao ganha o contraste que ela precisa ter.",
    "  foto = clamp((foto - 0.015) * 1.42, 0.0, 1.0);",
    "  float cinza = dot(foto, vec3(0.299, 0.587, 0.114));",
    "  foto = clamp(mix(vec3(cinza), foto, 1.55), 0.0, 1.0);",
    "  cinza = dot(foto, vec3(0.299, 0.587, 0.114));",
    "",
    "  // Separacao em quatro chapas, com remocao de cor sob o preto: o",
    "  // que as tres cores fariam junto vira preto, que imprime melhor e",
    "  // gasta menos tinta. E o que a oficina fazia de verdade.",
    "  vec3 cmy = 1.0 - foto;",
    "  float k = min(cmy.r, min(cmy.g, cmy.b));",
    "  cmy = (cmy - k) / max(1.0 - k, 0.001);",
    "",
    "  // O fundo da foto e quase preto: impresso fiel, ele viraria uma",
    "  // mancha solida que engole o papel e esconde a forma da fumaca.",
    "  // Entao a cobertura e pesada pela luz: o fundo entra fraco, a",
    "  // coruja entra cheia. E ela que tem de saltar, nao o retangulo.",
    "  float cobertura = mascara * (0.22 + 0.78 * smoothstep(0.03, 0.40, cinza));",
    "",
    "  // O passo em pixel de DISPOSITIVO faria a reticula encolher pela",
    "  // metade numa tela de densidade 2 e virar gerador de moire no",
    "  // projetor. Em pixel de CSS o ponto tem o mesmo tamanho fisico",
    "  // em qualquer maquina.",
    "  float passo = 6.0 * uDpr;",
    "  float pc = chapa(cmy.r * cobertura, radians(15.0), passo, vec2( 0.0,  0.0));",
    "  float pm = chapa(cmy.g * cobertura, radians(75.0), passo, vec2( 1.9, -1.3));",
    "  float py = chapa(cmy.b * cobertura, radians(0.0),  passo, vec2(-1.4,  2.1));",
    "  float pk = chapa(k * cobertura * 0.82, radians(45.0), passo, vec2( 0.8,  0.9));",
    "",
    "  vec3 cor = PAPEL;",
    "  cor *= mix(vec3(1.0), TINTA_C, pc);",
    "  cor *= mix(vec3(1.0), TINTA_M, pm);",
    "  cor *= mix(vec3(1.0), TINTA_Y, py);",
    "  cor *= mix(vec3(1.0), TINTA_K, pk);",
    "",
  ...
  gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, 1, 1, 0, gl.RGBA, gl.UNSIGNED_BYTE,
                new Uint8Array([233, 227, 211, 255]));   // papel, nao o azul do tema antigo
